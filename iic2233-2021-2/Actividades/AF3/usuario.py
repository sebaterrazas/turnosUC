...
class Usuario:
    ...
    def ver_videos(self, nombre_lista):
        lista_pelis = self.listas_reproduccion[nombre_lista]
        for peli in lista_pelis:
            peli.reproducir()
            calificacion = self.calificar_pelicula(peli)
            yield calificacion, peli.rankings
        print('Se llegó al final de la lista de reproducción', nombre_lista)
